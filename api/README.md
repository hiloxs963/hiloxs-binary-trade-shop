# HILOXS API foundation

This package is the infrastructure-only backend foundation for HILOXS. It is deliberately isolated
...
errors, PostgreSQL pooling, Drizzle migrations, liveness/readiness endpoints, and CI. It does not
provide application domain features.
...
No authentication, payment, cloud database, CORS, or backend-as-a-service SDK is installed.
...
- Cross-origin browser access is disabled because Phase 1 has no browser consumer. Phase 2 should
  explicitly allow `https://hiloxs.co.ke` and selected local development origins, never `*`.
...
- Authorization, cookies, passwords, tokens, secrets, and database URLs are redacted from structured
  logs. Client errors receive only stable codes and safe messages.
- The trust-boundary decision is recorded in `../docs/adr/0001-backend-trust-boundary.md`.

## Intentionally not implemented

There are no users, sessions, roles, authentication, products, carts, orders, checkout, payments,
M-Pesa integration, wallets, ledgers, withdrawals, referrals, bonuses, sellers, document storage,
training APIs, or trading APIs. The only table is the infrastructure-owned `system_metadata` table.
